...
- **Proof obligations** - Generated in-process on open and on save, judged against the stored proofs the way `rossi prove` does, anchored on their source elements, and served through `rossi/proofObligations` and the `$/rossi/proofStatus` push
...
`rossi/proofObligations` takes `{ textDocument: { uri } }` and returns the
open document's proof obligations, each as
...
recomputed. A client that ignores it loses nothing but the live update.
