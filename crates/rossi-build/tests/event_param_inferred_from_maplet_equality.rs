...
//! leaves. Rodin parity — verified against `arinc653.zip/Mach_IPC.bcm`,
//! where `read_sampling_message`'s `grd04` types parameters `m` and
//! `t` via `m ↦ t = msgspace_of_samplingports(port)`.
...
<org.eventb.core.event name="_ev_read" org.eventb.core.convergence="0" org.eventb.core.extended="false" org.eventb.core.label="read_sampling_message">
...
fn parameter_typed_via_maplet_equality_against_function_application() {
    ...
    let ev = v
        .events
        .get("read_sampling_message")
        .expect("read_sampling_message event present");
    assert!(
        ev.accurate,
