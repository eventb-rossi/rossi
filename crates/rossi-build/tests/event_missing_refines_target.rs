...
//! — verified against `tutorial_fx2-tut2.zip/E-ALGO.bcm`, where the
//! refining events `computing`/`step` target non-existent abstract
//! events in `D-PREALGO` and Rodin emits a `.bcm` that contains only
//! `INITIALISATION` while still marking the file `accurate="true"`.
