...
pub(crate) fn collect_binder_types(
    ...
    match pred {
        ...
        // `∀a,t · a ∈ policy_of_tcb(t) ⇔ a ∈ POLICYSETS` — the left
    ...
    match predicate {
        Predicate::Comparison { op, left, right } => match op {
            ComparisonOp::In | ComparisonOp::NotIn => {
                ...
                // (Covers `tcb`'s `a ∈ POLICYSETS` where `a : ACCESSES`
            ...
            // (Covers `evbt_wellcommented`'s `maa>1`, `mii>−10`.)
        ...
        //   - `⇔` — `tcb`'s `∀a,t · a∈policy_of_tcb(t) ⇔ a∈POLICYSETS`
        //     (POLICYSETS typed via the LHS)
        //   - `∨` — `aman_abstraction`'s
        //     `new_abstract_blockedTime = X ∨ new_abstract_blockedTime = Y`
        //   - `⇒` — `arinc653`'s `(A ⇒ p = {q}) ∧ (¬A ⇒ p = ∅)`
...
mod tests {
    ...
    fn range_restriction_preserves_relation_type() {
        ...
        // type as `checked`. From comp1216_hotel_reception.
        let mut env = TypeEnv::new();
    ...
    fn parameter_typed_via_implication_pair_consequent() {
        // arinc653-style guard: `(A ⇒ p = {q}) ∧ (¬A ⇒ p = ∅)` where the
    ...
    fn parameter_typed_via_maplet_equality_against_function_application() {
        // arinc653-style: `m ↦ t = f(port)` where
    ...
    fn parameter_typed_via_intersection_with_typed_sibling_operand() {
        // ertms-hl3-abz2018 shape: `vss_set ∩ ma[{tr}] = ∅`. The
