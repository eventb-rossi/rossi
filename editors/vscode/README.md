...
- `rossi.rodin.provingPerspective`: Open Rodin in its Proving perspective rather than the Event-B modelling one when the `Open in Rodin` lens runs; on a workspace Rodin has opened before this needs the bridge plug-in, which switches the perspective inside the running instance, since Eclipse otherwise restores the perspective last active there (default: `false`)
...
Every open `.eventb` file has its proof obligations generated by the language
server on open and on save, with the same generator `rossi build` uses, so the
list is what Rodin would show. Each obligation is judged against the stored
proof for it, looked for in the shared Rodin workspace project first and next
...
closed-over-total count per group. Clicking one reveals the invariant, guard,
action or axiom it is about and opens its sequent beside the editor: a
