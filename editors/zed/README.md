...
> token core generated by `rossi gen-grammars`), pinned in `extension.toml`.
...
```bash
cargo run -p rossi-cli -- gen-grammars        # updates grammar.js, highlights.scm (+ this extension's copy), tokens.json, snippets
```
