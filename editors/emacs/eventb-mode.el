...
(defvar eventb-font-lock-keywords
  `((,eventb-label-regexp . font-lock-preprocessor-face)
    ...
    ("\\<[Ee][Vv][Ee][Nn][Tt]\\s-+\\([[:alpha:]_][[:alnum:]_]*\\(?:-[[:alnum:]_]+\\)*\\)" 1 font-lock-function-name-face)
    ("\\<\\(?:[Cc][Oo][Nn][Tt][Ee][Xx][Tt]\\|[Mm][Aa][Cc][Hh][Ii][Nn][Ee]\\)\\s-+\\([[:alpha:]_][[:alnum:]_]*\\(?:-[[:alnum:]_]+\\)*\\)" 1 font-lock-type-face)
